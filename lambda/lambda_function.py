...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        reprompt_text = "I was born on November 6th, 2014. When were you born?"
        return (
...
class CaptureZodiacSignIntentHandler(AbstractRequestHandler):
    """Handler for Capture Zodiac Sign Intent."""
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
    ...
    def handle(self, handler_input):
        ...
        day = slots["day"].value
        url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRfuTxlsgkQZgdyo1wKFBSRU8RTtCV5PsCT6V4DRBpW3fMHNXTj-4sikUfsQmNZLg/pub?gid=14074612&single=true&output=csv"
        ...
        for index,row in df.iterrows():
            if self.filter(row['Start']) <= usr_dob <= self.filter(row['End']):
                zodiac = row['Zodiac']
        speak_output = "I see you were born on the {day} of {month} {year}, which means that your zodiac sign will be {zodiac}.".format(month=month,day=day,year=year,zodaic=zodiac)
